...
class TestSimulationEnv(unittest.TestCase):
    def test_prev_step_returns_on_scheduling_time(self):
        ...
        env = SimulationEnv(setup_default_data_strategies(), start_datetime=start_datetime, scheduling_time=scheduling_time, start_tick=24)
    ...
    def test_24_hours_pass_between_consecutive_steps(self):
        start_datetime = datetime(2022, 1, 1)
        env = SimulationEnv(setup_default_data_strategies(), start_datetime=start_datetime, start_tick=24)
        env._run_in_random_order = lambda *args: None
        clock = env._clock
    ...
    def test_time_flows_independently_in_envs(self):
        start_datetime = datetime(2022, 1, 1)
        env1 = SimulationEnv(setup_default_data_strategies(), start_datetime=start_datetime, start_tick=24)
        env2 = SimulationEnv(setup_default_data_strategies(), start_datetime=start_datetime, start_tick=24)
