...
class TestSimulationEnv(unittest.TestCase):
    ...
    def test_is_now_scheduling_time(self):
        start_datetime = datetime(2022, 1, 1)
        scheduling_time = start_datetime + timedelta(hours=1)
        env = SimulationEnv(start_datetime=start_datetime, scheduling_time=scheduling_time.time())
    ...
    def test_is_now_action_replacement_time(self):
        start_datetime = datetime(2022, 1, 1)
        action_replacement_time = start_datetime + timedelta(hours=1)
        env = SimulationEnv(start_datetime=start_datetime, action_replacement_time=action_replacement_time.time())
